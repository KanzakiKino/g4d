...
void main ()
{
    ...
    auto ngon = new RegularNgonElement!15;

    auto textShader       = new Alphaf3DShader;
    textShader.color      = vec4(1,0.5,1,1);
    textShader.transform  = vec3(1,1,1);
    textShader.rotation   = vec3(0f,0f,0f);
    textShader.projection = mat4.transform( 1f/320f, 1f/240f, 1f );

    auto fillShader       = new Fill3DShader;
    fillShader.color      = vec4(1,1,1,0.5);
    fillShader.transform  = vec3(1,1,1);
    fillShader.rotation   = vec3(0f,0f,0f);
    fillShader.projection = mat4.transform( 1f/320f, 1f/240f, 1f );
    ...
    while ( win.alive )
    {
        ...
        fillShader.use();
        fillShader.translate = vec3(0,0,0);
        ngon.resize( frame%300 );
        ngon.draw( fillShader );

        textShader.use();
        textShader.translate = vec3(-320,240,-1);
